...
export class Service {
            ...
            this.http.get(this.config.url + '/wp-admin/admin-ajax.php?action=mstoreapp-keys', this.config.options).map(res => res.json()).subscribe(data => {
                ...
                this.reqhttp.get(this.config.setUrl('GET', '/wp-json/wc/v2/products/categories?', {per_page: 100}), {}, {}).then(data => {
                    ...
                    if (this.values.isLoggedIn) {
                        ...
                        this.reqhttp.clearCookies();
                        this.reqhttp.get(this.config.setUrl('GET', '/wp-json/wc/v2/orders?', this.filter), {}, {}).then(data => {
                            this.values.orders = JSON.parse(data.data);
                            resolve(this.orders);
                        });
                    }
                    this.loadCart();
    ...
    getOrders() {
        if (this.values.isLoggedIn) {
            ...
            return new Promise(resolve => {
                this.reqhttp.clearCookies();
                this.reqhttp.get(this.config.setUrl('GET', '/wp-json/wc/v2/orders?', this.orderFilter), {}, {}).then(data => {
